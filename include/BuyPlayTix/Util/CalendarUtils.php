...
class CalendarUtils {
	...
	public static function create_minical($production, $rawDates = array()) {
		...
		foreach($rawDates as $rawDate) {
			...
			$months[date("Ym", $time)][] = date("Y-m-d", $time);
		}
		ksort($months);
			...
			for($i = 1; $i <= $days_in_month; $i++) {
				...
				foreach($values as $value) {
					if($value == $cur_date) {
						...
						$link = '<a href="#" onclick="$(\'.hashSelected\').removeClass(\'hashSelected\'); $(\'a[name=' . $cur_date . ']\').parents(\'.ticket\').addClass(\'hashSelected\'); $.scrollTo(\'.hashSelected:first\', 800); return false;">';
					}
				}
